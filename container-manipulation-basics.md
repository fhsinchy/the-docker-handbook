...
Another very popular option of the `run` command is the `--detach` `-d` option. In the example above, in order for the container to keep running, you had to keep the terminal window open. Closing the terminal window also stopped the running container.
...
The `ls` command only lists the containers that are currently running on your system. In order to list out the containers that has run in the past you can use the `--all` or `-a` option.
...
If you've been following the article exactly as written so far, you should only see the `hello-dock-container` and `hello-dock-container-2` in the list. I would suggest stopping and removing both containers before going to the next section.
...
The [fhsinchy/rmbyext](https://hub.docker.com/r/fhsinchy/rmbyext) image behaves in a similar manner. To delete files using this image instead of the program itself, you can execute the following command:
...
The `-v $(pwd):/zone` is a way to grant the image access to your local file system using a volume. Volumes will be explained in great detail later on, for now just understand that this is a way of granting your container access to your local file system.
